...
export const Route = createFileRoute("/vbucks-missions")({
  loader: ({ context }) => {
    context.queryClient.ensureQueryData({
      queryKey: ["missions"],
      queryFn: async () => {
        const { getMissions } = await import("@/services/missions.api");
        return getMissions();
      },
    });
  },
  head: () => ({
    meta: [
      ...
      { property: "og:image", content: "https://hawkbucks.pages.dev/HawkBucks-logo.jpg" },
      ...
      { name: "twitter:image", content: "https://hawkbucks.pages.dev/HawkBucks-logo.jpg" },
    ],
